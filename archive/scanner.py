...
class AlgorithmScanner:
    def __init__(self):
        ...
        self.ignore_dirs = {'.git', 'venv', '__pycache__', 'node_modules', '.idea', '.vscode', 'build', 'dist'}
            ...
            for file in files:
                ...
                if file_path.suffix in self.supported_extensions:
                    ...
                    if file_result and file_result["functions"]:
                        ...
                        inventory["meta"]["algorithms_found"] += sum(len(f["algorithms"]) for f in file_result["functions"])
    ...
    def _analyze_function_node(self, node, code_bytes, file_data):
        ...
        for alg_name, detector in detectors:
            try:
                ...
                is_detected = result.get(f"is_{alg_name.lower().replace(' ', '_')}") or result.get(f"is_{alg_name.lower()}")
