...
<body>
    <header>
        ...
        <div class="controls">
            ...
            <button id="heat-btn" onclick="toggleHeatmap()">🌡️ Enable Heatmap</button>
        </div>
    </header>
    <div class="container">
    ...
    <script>
        ...
        function toggleHeatmap() {{
            ...
            graphData.nodes.forEach(n => {{
                const el = document.getElementById(`node-${{n.id}}`);
                if (isHeatmapMode) {{
                    const score = n.heat_score || 0;
                    // Green (120) -> Red (0)
                    const hue = 120 * (1 - score);
                    el.style.background = `linear-gradient(135deg, hsl(${{hue}}, 70%, 40%), hsl(${{hue}}, 70%, 30%))`;
                    el.style.borderColor = `hsl(${{hue}}, 80%, 60%)`;
                    el.style.boxShadow = `0 4px 10px hsla(${{hue}}, 70%, 10%, 0.5)`;
                }} else {{
                    el.style.background = '';
